export default function HospitalCard({ hospital, city, onEstimate }) {
  return <article className={`hospital ${hospital.best ? 'best' : ''}`}>
    ...
    <h3>{hospital.name}</h3><div className="place">{hospital.distance} · {city}</div>
    <div className="price-label">Estimated package price</div><div className="price">{hospital.price}</div>
